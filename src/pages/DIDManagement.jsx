          ...
          <table className="w-full min-w-[960px] text-sm">
            <thead>
              <tr className="bg-gray-50 text-left border-b border-border">
                <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500 whitespace-nowrap">DID Number</th>
                ...
                <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500 whitespace-nowrap">Asterisk Trunk</th>
                <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500 whitespace-nowrap">Calls Today</th>
              ...
              {loading ? (
                ...
                dids.map((did) => (
                  <tr key={did.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-gray-600 max-w-[10rem] truncate">{did.buyerName || '—'}</td>
                    <td className="px-5 py-3.5 text-gray-600 max-w-[12rem]">
                      {did.customerName ? (
                        <div className="min-w-0">
                          <span className="block truncate">{did.customerName}</span>
                          {did.displayNumber && (
                            <p className="text-xs text-gray-400 mt-0.5 truncate">Shows: {formatDidDisplay(did.displayNumber)}</p>
                          )}
                        </div>
                      ) : (
                        '—'
                    ...
                    <td className="px-5 py-3.5 text-gray-600 max-w-[10rem] truncate">{did.campaignName || 'Default'}</td>
                    <td className="px-5 py-3.5 text-gray-600 font-mono text-xs whitespace-nowrap">{did.trunk}</td>
                    <td className="px-5 py-3.5 text-gray-600 whitespace-nowrap">{did.callsToday ?? 0}</td>
                    <td className="px-5 py-3.5">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <button
            ...
            aria-label="Close"
          />
          <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-t-2xl sm:rounded-2xl shadow-xl border border-border p-5 sm:p-6">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:gap-3 pt-2">
                <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2.5 text-sm border rounded-xl">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2.5 text-sm font-bold bg-brand rounded-xl">
                  Save DID
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-t-2xl sm:rounded-2xl shadow-xl border border-border p-5 sm:p-6">
            ...
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:gap-3 pt-2">
                <button type="button" onClick={() => setEditDid(null)} className="px-4 py-2.5 text-sm border rounded-xl">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2.5 text-sm font-bold bg-brand rounded-xl">
                  Save Changes
